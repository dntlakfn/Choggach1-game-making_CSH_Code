...
{
    [CustomEditor(typeof(StageSubTypeListSO))]
    public class StageSubListSOEditor : UnityEditor.Editor
    {
        ...
        {
            StageSubTypeListSO list = target as StageSubTypeListSO;

            int index = 0;
            string enumString = string.Join(", ", list.subTypes.Select(so =>
            {
                so.stageSubTypeIndex = index;
